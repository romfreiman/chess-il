...
|----|-------------|------------------|
| SCRP-01 | Backend can fetch HTML from chess.org.il player page given a player ID | Verified: GET `https://www.chess.org.il/Players/Player.aspx?Id={id}` returns UTF-8 HTML. Use axios with `responseType: 'text'`, custom User-Agent header. Non-existent IDs return HTTP 500. |
| SCRP-02 | Scraper extracts player info fields (name, ID, FIDE ID, club, birth year, rating, expected rating, rank, grade, license expiry) | Verified: All fields found in `div.full-block ul li` elements with Hebrew text labels. FIDE ID may be empty (link text empty). Expected rating is inside a link to `ExpectedRating.aspx`. |
| SCRP-03 | Scraper extracts tournament history rows (date, update date, tournament name/URL, games, points, performance, result, W/D/L, rating change, pending status) | Verified: GridView table `#ctl00_ContentPlaceHolder1_PlayerFormView_TournamentsGridView` with 8 columns. 20 rows per page, server-side pagination. Rating change format is `VALUE+`/`VALUE-` (sign after number). |
...
| CACH-01 | Scraped player data is stored in Supabase `players` table as JSONB | Supabase upsert with `player_id` as primary key. Initialize client at module scope for reuse across warm invocations. |
| CACH-02 | Cached data is returned if updated within last 24 hours | Query: check `updated_at` timestamp against `now() - interval '24 hours'`. |
...
| API-03 | API is accessible from frontend (same-origin or CORS enabled) | Same-origin on Netlify (both SPA and functions served from same domain). Add CORS headers for local development. |
...
|---------|---------|---------|--------------|
| Express | 4.21.x | REST API routing and middleware | Lightweight, well-understood, works with serverless-http |
| Cheerio | 1.2.0 | HTML parsing and data extraction | jQuery-like API for server-rendered pages, no browser overhead, encoding detection built-in |
| axios | 1.15.x | HTTP client for fetching chess.org.il pages | Supports custom headers, response types, timeouts, AbortController |
| @supabase/supabase-js | 2.103.x | Database client for caching layer | REST-based (PostgREST), no direct Postgres connections, serverless-safe |
| serverless-http | 4.0.0 | Wraps Express for Netlify Functions | Official recommended approach per Netlify docs |
...
|---------|---------|---------|-------------|
| @netlify/functions | 5.2.0 | Netlify Functions types and utilities | Function handler typing |
| dotenv | 16.x | Environment variable loading | Local development only (Supabase URL/key) |
| vitest | 4.1.x | Test framework | Unit tests for parser, integration tests for API |
...
npm install -D typescript @types/express @netlify/functions vitest dotenv
```

**Version verification:** Versions confirmed against npm registry on 2026-04-20. Node.js v22.22.0 available on development machine. Netlify CLI not installed -- must be added.
...
export function parsePlayerPage(html: string): PlayerData {
  ...
  return { player: { name, id: playerId, /* ... */ }, tournaments };
}
...
export async function getPlayer(req: Request, res: Response) {
  ...
  if (!forceRefresh) {
    ...
    if (cached && !isStale(cached.updated_at)) {
      return res.json({ player: cached.data.player, tournaments: cached.data.tournaments, meta: { cached: true, stale: false, scrapedAt: cached.data.scrapedAt, cachedAt: cached.updated_at } });
    }
  }
  ...
  try {
    ...
    return res.json({ ...data, meta: { cached: false, stale: false, scrapedAt: new Date().toISOString(), cachedAt: new Date().toISOString() } });
  } catch (scrapeError) {
    // 3. Fallback to stale cache
    const staleCache = await getCachedPlayer(id);
    if (staleCache) {
      return res.json({ ...staleCache.data, meta: { cached: true, stale: true, scrapedAt: staleCache.data.scrapedAt, cachedAt: staleCache.updated_at } });
    }
...
| Date parsing for dd/MM/yyyy | Custom string splitting | Still manual -- but wrap in a utility function | No library needed, but centralize the logic to avoid inconsistent parsing |

**Key insight:** The scraping domain is deceptively simple -- "just parse some HTML" -- but encoding, encoding detection, malformed HTML, selector fragility, and date format assumptions create a minefield. Cheerio handles the HTML complexity; the remaining traps are in data interpretation (date formats, rating change sign format, optional vs missing fields).

## Live HTML Structure Analysis (HIGH confidence -- verified 2026-04-20)

### Key Findings from chess.org.il Inspection

**Encoding:** UTF-8 confirmed via both `Content-Type: text/html; charset=utf-8` header and `<meta charset="utf-8" />` tag. No Windows-1255 fallback needed.
...
<div class="full-block clearfix">
  <ul>
    ...
    <li>מספר שחקן פיד"ה: <a href="https://ratings.fide.com/profile/2875080" target="_blank">2875080</a></li>
    <li>מדינה: <a href="#">ISR</a></li>
...
```
Player name is in `div.p-profile .section-title h2`. Grade is in `div.p-profile p`.

**Rating and Ranking** are in separate `div.full-block` containers:
- Rating: `מד כושר ישראלי<span>: 1476 (צפוי: </span><a href="ExpectedRating.aspx?Id=205001">1475</a>`
- Rank: `דירוג בישראל<span>: 2872</span>`

**FIDE ID Edge Case:** Player 210498 has the FIDE field present but with empty link text: `<a href="https://ratings.fide.com/profile/" target="_blank"></a>`. The scraper must check link text is non-empty to determine if a FIDE ID exists.
...
- 8 columns: Start Date, Update Date, Tournament Name (linked), Games, Points, Performance, Result, Rating Change
- 20 rows per page with server-side pagination via `__doPostBack`
- Pagination row has class `GridPager`
...
- `0` (no change, plain text without span)

The parser must reverse this format: extract number, check trailing `+`/`-`, convert to standard float.

**Update Date Formats:**
- Pending: `בעדכון הבא` (literal text)
- Processed: `עדכון DD/MM/YYYY` (e.g., `עדכון 01/04/2026`)

**Result Format:** `+W-L=D` (e.g., `+3-1=0` means 3 wins, 1 loss, 0 draws)

**Non-existent Player:** HTTP 500 response with generic ASP.NET "Runtime Error" page. Title contains "Runtime Error". Scraper must check HTTP status code and detect this error page.
...
**Why it happens:** This is a Hebrew/RTL display convention where the sign is shown after the number.
**How to avoid:** Parse with regex: `/^([\d.]+)([+-])$/` -- extract number and trailing sign, then apply sign to float.
...
**How to avoid:** Check for HTTP 500 status AND detect "Runtime Error" in the response body. Return a user-friendly 404 error.
...
**How to avoid:** Always split on `/` and construct manually: `const [d, m, y] = dateStr.split('/'); return `${y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}`;`
...
**What goes wrong:** Routes work locally but return 404 in production because the Netlify redirect strips `/api` but Express expects it.
...
**How to avoid:** Mount Express routes at `/api/player/:id` (keeping the `/api` prefix). The redirect passes the full path after `/api/` as `:splat`. Test with `netlify dev` from the start.
**Warning signs:** All routes returning 404 in production.
...
**How to avoid:** Use `service_role` key only in serverless functions (via Netlify environment variables). Add `.env` to `.gitignore` immediately. Create `.env.example` with placeholder values.
...
function parsePlayerInfo($: cheerio.CheerioAPI): PlayerInfo {
  ...
  // Grade: paragraph after the name section-title
  const grade = $('.p-profile > p').first().text().trim();
  ...
  // Rating: "מד כושר ישראלי: 1476 (צפוי: 1475)"
  const ratingLi = findLiByLabel(allListItems, $, 'מד כושר ישראלי');
  const ratingText = ratingLi.find('span').first().text();
  const rating = parseInt(ratingText.replace(/[^0-9]/g, ''), 10);
  const expectedRating = parseInt(ratingLi.find('a').text().trim(), 10) || null;

  // Rank: "דירוג בישראל: 2872"
  const rankText = extractByLabel(allListItems, $, 'דירוג בישראל');
...
function parseTournamentTable($: cheerio.CheerioAPI): TournamentEntry[] {
  ...
  table.find('tbody tr').not('.GridPager').each((_, row) => {
    ...
    const ratingChangeRaw = cells.eq(7).text().trim();
...
```typescript
// Source: Supabase JS v2 documentation
...
|--------------|------------------|--------------|--------|
| Cheerio pre-1.0 (`cheerio.load()` deprecated static) | Cheerio 1.2.0 (`import * as cheerio from 'cheerio'`) | 2024 | Import style changed; `loadBuffer` and `fromURL` added |
...
| supabase-js v1 `.insert()` returns rows | supabase-js v2 `.upsert()` requires `.select()` to return rows | 2023 | Must chain `.select()` if you need the inserted/updated row back |
| `node-fetch` for HTTP | Native `fetch` in Node 18+ | 2023 | axios still preferred for better timeout/header API, but native fetch is an option |
| Netlify Functions v1 | Netlify Functions v2 (updated March 2025) | 2025 | New function format available but v1 pattern (serverless-http) still works |
...
1. **Tournament Pagination (20 per page limit)**
   - What we know: The initial GET returns only 20 most recent tournaments. Full history requires `__doPostBack` with ViewState (~37KB POST body). Player 205001 has ~100 tournaments across 5 pages.
...
3. **Supabase Table Design**
   - What we know: Single `players` table with `player_id`, `data` (JSONB), `updated_at`. Primary key on `player_id`.
...
| Netlify CLI | Local development (`netlify dev`) | No | -- | Install via `npm install -g netlify-cli` |
...
- **Live chess.org.il inspection** -- Fetched `https://www.chess.org.il/Players/Player.aspx?Id=205001` and `?Id=210498` on 2026-04-20. Verified encoding (UTF-8), HTML structure, field locations, tournament table format, pagination, error response for invalid IDs.
- **npm registry** -- Verified current package versions: cheerio 1.2.0, express 5.2.1 (4.x stable recommended), axios 1.15.1, @supabase/supabase-js 2.103.3, serverless-http 4.0.0, typescript 6.0.3, vitest 4.1.4.
- [Netlify Express setup guide](https://docs.netlify.com/build/frameworks/framework-setup-guides/express/) -- File structure, netlify.toml config, serverless-http wrapping pattern
- [Netlify Functions overview](https://docs.netlify.com/build/functions/overview/) -- 60s sync timeout, 1024MB memory, 6MB payload limit
- [Supabase JS upsert docs](https://supabase.com/docs/reference/javascript/upsert) -- v2 upsert API, onConflict, `.select()` chaining
...
- [MDN TextDecoder](https://developer.mozilla.org/en-US/docs/Web/API/TextDecoder) -- windows-1255 support confirmed (not needed since site is UTF-8)
...
- Pitfalls: HIGH -- encoding verified as UTF-8 (simplifies D-02), HTML structure inspected, error behavior tested
